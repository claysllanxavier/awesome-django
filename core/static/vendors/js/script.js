...
jQuery(document).ready(function () {
  ...
  $(".nif").mask(CpfCnpjMaskBehavior, cpfCnpjpOptions);
  ...
  $("form").on("submit", function (event) {
    $("input[type=text]").each(function () {
      if (!$(this).hasClass("datefield")) $(this).unmask();
    });
  });

  $(".nif").on("change", function () {
    var value = $(this).cleanVal();
    $.get("/api/v1/contatos/pessoa/?nif=" + value).done(function (data) {
      var item = data[0];
      if (item) {
        $("input, select, textarea").each(function () {
          if (item[this.name]) $(this).val(item[this.name]);
        });
      }
    });
  });
